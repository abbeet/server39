<?php
	...
	{		
		if ($err != true) 
		{
			...
			#hapus dulu datanya
			$sql = "DELETE FROM thbp_kak_kegiatan WHERE th = '".$tahun."' and kdunitkerja = '".$kdunitkerja."'";
			mysql_query($sql);
			
			$sql = "DELETE FROM thbp_kak_output WHERE th = '".$tahun."' and kdunitkerja = '".$kdunitkerja."'";
			mysql_query($sql);
			...
			$row3 = mysql_fetch_array($qu3);
					...
					$sql = "INSERT INTO thbp_kak_kegiatan VALUES ('','".$tahun."','".$row2['KDUNITKERJA']."','".$row['KDDEPT']."','".$row['KDUNIT']."','".$row3['KDPROGRAM']."','".$row['KDGIAT']."','','".$row['anggaran_dipa']."','')";# echo $sql.";<br>";
					mysql_query($sql);
				#}
				
       			$sql = "SELECT KDOUTPUT, SUM(JUMLAH) as dipa_output FROM d_item WHERE THANG='$tahun' and KDGIAT='$row[KDGIAT]'  GROUP BY KDOUTPUT ORDER BY KDOUTPUT"; 
				$qu4 = mysql_query($sql);
