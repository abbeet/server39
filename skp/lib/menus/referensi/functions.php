...
function calculate_tf_en($arr_kata)
{
	...
	function nm_program($kode) {
		$kddept = substr($kode,0,3);
		$kdunit = substr($kode,3,2);
		$kdprogram = substr($kode,5,2);
		$data = mysql_query("select nmprogram from tb_program where kddept='$kddept' and kdunit='$kdunit' and kdprogram='$kdprogram'");
		$rdata = mysql_fetch_array($data);
		$result = trim($rdata['nmprogram']);
		return $result;
	}
	
	function nm_giat($kode) {
		$data = mysql_query("select nmgiat from tb_giat where kdgiat='$kdgiat'");
		$rdata = mysql_fetch_array($data);
