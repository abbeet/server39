...
<form action="index.php?p=<?php echo $_GET['p']; ?>&u=<?php echo $u; ?>" method="post" name="form">
	<table cellspacing="1" class="admintable">
		<tr>
			<td class="key">Nama Unit Kerja</td>
			<td>
				<select name="kdunit" disabled="disabled"><?php
					$sql = "SELECT * FROM tb_unitkerja ORDER BY kdunit";
					$aUnitKerja = mysql_query($sql);
					
					while ($UnitKerja = mysql_fetch_array($aUnitKerja))
					{ ?>
						<option value="<?php echo $UnitKerja['kdunit']; ?>" <?php if ($u == $UnitKerja['kdunit']) echo "selected"; ?>><?php echo $UnitKerja['nama_unit_kerja']; ?></option><?php
					} ?>
				</select>
		...
		<tr>
			...
			<td>			
				<div class="button2-right">
					<div class="prev">
						<a onclick="Back('index.php?p=33')">Batal</a>
					</div>
				</div>
		...
		{ ?>
			...
			foreach ($col[0] as $k=>$val) {
				...
				<tr class="<?php echo $class ?>">
					...
					<td align="center">
						<a href="index.php?p=68&u=<?php echo $u; ?>&q=<?php echo $col[0][$k]; ?>" title="Edit">
							<img src="css/images/edit_f2.png" border="0" width="16" height="16">
						</a>
					</td>
					<td align="center">
						<a href="index.php?p=90&u=<?php echo $u; ?>&q=<?php echo $col[0][$k]; ?>" title="Delete">
