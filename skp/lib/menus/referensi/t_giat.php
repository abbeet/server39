<?php
	...
	$query = "SELECT COUNT(*) as num FROM $table WHERE KdGiat LIKE '%$cari%' OR NmGiat LIKE '%$cari%'";
	...
	$oList = mysql_query("SELECT * FROM $table WHERE KdGiat LIKE '%$cari%' OR NmGiat LIKE '%$cari%' ORDER BY KDPROGRAM desc,KdGiat LIMIT $start, $limit");
...
<div align="right">
	<form action="" method="get">
		...
		<a href="http://183.91.67.3/sipristek/index.php?p=83">Reset</a>
	</form>
</div><br />
<table width="652" cellpadding="1" class="adminlist">
  <thead>
    <tr> 
      ...
      <th width="9%">Kode<br>Kegiatan</th>
      <th width="44%">Kegiatan</th>
    </tr>
  </thead>
  <tbody>
    <?php
		if ($count == 0) { ?>
    <tr>
      <td align="center" colspan="<?php echo count($field)+1 ?>">Tidak ada data!</td>
    </tr>
    <?php
		}
		else {
    ...
    <tr class="<?php echo $class ?>"> 
      <td align="center"><?php echo $limit*($pagess-1)+($k+1); ?></td>
      <td align="left"> 
        <?php if($col[2][$k].$col[3][$k].$col[4][$k] <> $col[2][$k-1].$col[3][$k-1].$col[4][$k-1]){?>
        <?php echo nm_program($col[2][$k].$col[3][$k].$col[4][$k]) ?> 
        <?php }?>
      </td>
