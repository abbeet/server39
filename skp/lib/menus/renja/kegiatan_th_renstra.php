<?php
	...
	{		
		if ($err != true) 
		{
			$sql = "SELECT DISTINCT kdgiat FROM m_ikk_kegiatan ORDER BY kdgiat"; 
			$qu = mysql_query($sql);
			...
			while ($row = mysql_fetch_array($qu))
			{
				...
				$sql = "SELECT dana_".$tahun." as dana FROM m_aldana_kegiatan WHERE kdgiat = '".$row['kdgiat']."'";
				$qu3 = mysql_query($sql);
				...
				#{
					$sql = "INSERT INTO thbp_kak_kegiatan VALUES ('','".$tahun."','".$row2['kdunitkerja']."','".$row['kdgiat']."','".$row2['kddept']."','".$row2['kdunit']."','".$row2['kdprogram']."','".$row3['dana']."')";# echo $sql.";<br>";
					mysql_query($sql);
				#}
				
				$thnn = $tahun - 1;
				$sql = "SELECT kdoutput, target_".($thnn)." as volume1, target_".$tahun." as volume2, masuk_renja FROM m_ikk_kegiatan WHERE kdgiat = '".$row['kdgiat']."' AND masuk_renja = '1' ORDER BY kdoutput";
				$qu4 = mysql_query($sql);
