<?php
	...
	$query = "SELECT * FROM $table WHERE THANG LIKE '%$cari%' GROUP BY CONCAT(THANG,KDSATKER)";
	#echo $query;
	...
	$oList = mysql_query("SELECT * FROM $table WHERE THANG LIKE '%$cari%' GROUP BY CONCAT(THANG,KDSATKER) LIMIT $start, $limit");
		...
		if ($count == 0) { ?>
			...
			foreach ($col[0] as $k=>$val) {
				...
				<tr class="<?php echo $class ?>">
					...
					<td width="14%" align="center"><a href="index.php?p=120&kdsatker=<?php echo $col[1][$k] ?>&th=<?php echo $col[0][$k] ?>" title="Data Detil Kegiatan"><?php echo $col[1][$k] ?></a></td>
					<td width="47%" align="left"><?php echo nm_satker($col[1][$k]) ?></td>
					<td width="14%" align="right"><?php echo number_format(anggaran_satker($col[0][$k],$col[1][$k]),"0",",",".") ?></td>
				</tr><?php
			}
		} ?>
	</tbody>
