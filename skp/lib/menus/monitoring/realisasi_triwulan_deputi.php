...
?>
    <tr bordercolor="#999999" class="<?php echo $class ?>"> 
      <td width="6%" align="center"><font color="#FF0000"><?php echo $kddeputi ?></font></td>
      <td width="24%" align="left"><font color="#FF0000"><?php echo nm_unit($kddeputi.'000') ?></font></td>
      <td align="right" valign="top"><font color="#FF0000"><?php echo number_format($Deputi['pagu_deputi'],"0",",",".") ?></font></td>
      <td align="right" valign="top"><font color="#FF0000"><?php echo number_format($Deputi['rencana_deputi'],"0",",",".") ?></font></td>
      <td align="center" valign="top"><font color="#FF0000"><?php echo number_format(($Deputi['rencana_deputi']/$Deputi['pagu_deputi'])*100,"2",",",".") ?></font></td>
      <td align="right" valign="top"><font color="#FF0000"><?php echo number_format($real_deputi,"0",",",".") ?></font></td>
      <td align="center" valign="top"><font color="#FF0000"><?php echo number_format(($real_deputi/$Deputi['pagu_deputi'])*100,"2",",",".") ?></font></td>
    </tr>
    <?php 
if($kdtriwulan == 1 ){	
	$sql = "select KDSATKER, sum(JUMLAH) as pagu_satker, sum(JANUARI + PEBRUARI + MARET) as rencana_satker from $table WHERE THANG='$th' and left(kdunitkerja,3) = '$kddeputi' group by KDSATKER order by KDSATKER";
	
}elseif($kdtriwulan == 2 ){	
	$sql = "select KDSATKER, sum(JUMLAH) as pagu_satker, sum(JANUARI + PEBRUARI + MARET + APRIL + MEI + JUNI) as rencana_satker from $table WHERE THANG='$th' and left(kdunitkerja,3) = '$kddeputi' group by KDSATKER order by KDSATKER";

}elseif($kdtriwulan == 3 ){	
	$sql = "select KDSATKER, sum(JUMLAH) as pagu_satker, sum(JANUARI + PEBRUARI + MARET + APRIL + MEI + JUNI + JULI + AGUSTUS +
			SEPTEMBER) as rencana_satker from $table WHERE THANG='$th' and left(kdunitkerja,3) = '$kddeputi' group by KDSATKER order by KDSATKER";

}elseif($kdtriwulan == 4 ){	
	$sql = "select KDSATKER, sum(JUMLAH) as pagu_satker, sum(JANUARI + PEBRUARI + MARET + APRIL + MEI + JUNI + JULI + AGUSTUS +
			SEPTEMBER + OKTOBER + NOPEMBER + DESEMBER) as rencana_satker from $table WHERE THANG='$th' and left(kdunitkerja,3) = '$kddeputi' group by KDSATKER order by KDSATKER";
}
	$aSatker = mysql_query($sql);
	while ($Satker = mysql_fetch_array($aSatker))
	{
	$kdsatker =  $Satker['KDSATKER'] ;
	$real_satker = realisasi_satkerdeputi_tw($th,$kddeputi,$kdsatker,$kdtriwulan);
	?>
    <tr class="<?php echo $class ?>"> 
