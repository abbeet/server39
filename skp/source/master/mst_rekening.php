<?php
	...
	$ed_link = "index.php?p=481";
	$del_link = "index.php?p=482";
...
<?php
	...
	if ( $kdunit == '2320100' )
	{
		$query  = "SELECT COUNT(*) as num FROM $table WHERE (kdunitkerja LIKE '$kdunitkerja%' AND nama LIKE '%$cari%') 
				   OR ( kdunitkerja = '2320000' AND nama LIKE '%$cari%') ";
	}else{
		$query  = "SELECT COUNT(*) as num FROM $table WHERE kdunitkerja LIKE '$kdunitkerja%' AND nama LIKE '%$cari%'";
	}
	...
	echo $pagination;
?>
<a href="source/master/daftar_rekening_prn.php?kdunit=<?php echo $kdunit ?>" title="Cetak Daftar Rekening" target="_blank">
			  <img src="css/images/menu/icon-16-print.png" border="0" width="16" height="16"><font size="1">Cetak PDF</font></a>&nbsp;|&nbsp;
<a href="source/master/daftar_rekening_xls.php?kdunit=<?php echo $kdunit ?>" title="Cetak Daftar Rekening" target="_blank">
		...
		if ($count == 0) { ?>
			...
			foreach ($col[0] as $k=>$val) {
				...
				<tr class="<?php echo $class ?>">
					...
					<td align="left" valign="top"><?php echo nm_jabatan_ij($col[6][$k],$col[3][$k]) ?></td>
					...
					<a href="<?php echo $ed[$k] ?>&q=<?php echo $List['id'] ?>&pagess=<?php echo $pagess ?>&kdunit=<?php echo $kdunit ?>" title="Edit">
