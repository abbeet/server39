<?php	
	...
	$this->Cell($w1[4],$ln*2,'Nama Jabatan',$border,1,'C');
		}		
	}
	...
	$w1 = array(15,50,55,10,60,12);    
	$no = 0 ;
	$sql = "SELECT * FROM mst_skp WHERE left(kdunitkerja,4) = '$kdunit' and tahun = '$th' ORDER BY kdunitkerja,kdjabatan,kdgol";
	$qu = mysql_query($sql);
	...
	$arrJabatan   = $pdf->SplitToArray($w1[4],$ln,nm_jabatan_ij($row['kdjabatan']));
	...
	for($i=0;$i<$max;$i++)
	{
		...
		$pdf->Cell($w1[4],$ln,$arrJabatan[$i],'LR',1,'L');
