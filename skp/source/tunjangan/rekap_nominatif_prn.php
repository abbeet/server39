...
//	$w1 = array(10,50,35,35,35);
	...
	if ( $kdunit == '2320100' )
	{
		$oList = mysql_query("SELECT grade, count(nip) as jumlah, sum(tunker) as jml_tunker, sum(pajak_tunker) as jml_pajak FROM mst_tk WHERE tahun = '$th' and bulan = '$kdbulan' and ( kdunitkerja LIKE '$xkdunit%' OR kdunitkerja = '2320000' ) GROUP BY grade ORDER BY grade desc");
	}else{
		$oList = mysql_query("SELECT grade, count(nip) as jumlah, sum(tunker) as jml_tunker, sum(pajak_tunker) as jml_pajak FROM mst_tk WHERE tahun = '$th' and bulan = '$kdbulan' and kdunitkerja LIKE '$xkdunit%' GROUP BY grade ORDER BY grade desc");
	}
