<?php
	...
	$oList = mysql_query("SELECT nib,norec,sum(nil_terima) as jml_terima FROM $table WHERE tahun = '$th' and kdsatker = '$kdsatker' and bulan >= '$kdbulan1' and bulan <= '$kdbulan2' GROUP BY nib ORDER BY nib, grade desc");
	$no = 0 ;
	...
	header("Expires: 0");
?>
<table width="92%" cellpadding="1" border="1">
	<thead>
		...
		if ($count == 0) { ?>
			...
			foreach ($col[0] as $k=>$val) {
				...
				if ($k % 2 == 1) $class = "row0";
				    ...
				    <td align="right" valign="top"><?php if ( $col[0][$k] == 1 ) {?>
					<?php echo $total ?><?php }?></td>
			    </tr>
		<?php
		}
		} ?>
