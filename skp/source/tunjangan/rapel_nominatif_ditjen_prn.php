...
//	$w1 = array(10,50,35,35,35);
	...
	$oList = mysql_query("SELECT grade, sum(tunker) as jml_tunker, sum(pajak_tunker) as jml_pajak FROM mst_tk WHERE tahun = '$th' and bulan >= '$kdbulan1' and bulan <= '$kdbulan2' GROUP BY grade ORDER BY grade desc");
	...
	$total        += jmlpeg_bulan_grade_ditjen($th,$kdbulan2,$row['grade']) ;
	$arrNo		  = $pdf->SplitToArray($w1[0],$ln,$no.'.');
	$arrGrade	  = $pdf->SplitToArray($w1[1],$ln,'GRADE '.$row['grade']);
	$arrPenerima  = $pdf->SplitToArray($w1[2],$ln,number_format(jmlpeg_bulan_grade_ditjen($th,$kdbulan2,$row['grade']),"0",",","."));
