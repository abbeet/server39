<?php
	...
	{		
		if ($err != true) 
		{
			...
			$sql = "SELECT nib,tunker,pajak_tunker FROM mst_tk WHERE tahun = '$tahun' AND bulan = '$bulan' AND kdsatker = '$xusername'"; 
			$qu = mysql_query($sql);
										
			while ($row = mysql_fetch_array($qu))
			{
					$nib = $row['nib'];
					$tot_tunker += $row['tunker'];
					$tot_pajak_tunker += $row['pajak_tunker'];
					$sql_inp = "UPDATE mst_tk SET status = '1' , tgl_status = '$tgl_status' WHERE nib = '$nib' ";
					mysql_query($sql_inp);
				#}
				
			} 
			mysql_query("DELETE FROM rekap_tk  WHERE tahun = '$tahun' AND bulan = '$bulan' AND kdsatker = '$xusername' ");
			mysql_query("INSERT rekap_tk value('','$xusername','$tahun','$bulan','$tot_tunker','$tot_pajak_tunker','') ");
			?>
