...
for($i=1;$i<=13;$i++)
{
	...
	$oList = mysql_query("SELECT grade, count(nib) as jumlah, sum(tunker) as jml_tunker, sum(pajak_tunker) as jml_pajak FROM $table WHERE tahun = '$th' and kdsatker = '$kdsatker' and bulan = '$kdbulan' GROUP BY grade ORDER BY grade desc");
	...
	<form action="index.php?p=<?php echo $p ?>&kdsatker=<?php echo $kdsatker ?>&kdbulan=<?php echo $kdbulan ?>" method="post">
		...
		<select name="kdsatker">
                    ...
                    <?php
							...
							{
								$query = mysql_query("select left(nmsatker,60) as nama_satker, kdsatker from kd_satker where kdsatker='$xusername'");
							}
...
<!-- akhir dari pilih satker-->

<br />
<a href="source/tunjangan/rekap_nominatif_satker_xls.php?kdsatker=<?php echo $kdsatker ?>&th=<?php echo $th ?>" title="Cetak Rekap Daftar Nominatif Satker" target="_blank">
