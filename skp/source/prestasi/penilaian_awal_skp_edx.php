...
<?php
	...
	if (isset($form)) {		
		if ($err != true) {
			...
			$modifiedby = $xusername_sess;
				...
				echo $id."<br>";
					if ($id <> ''){
						$sqlx = "UPDATE dtl_penilaian_skp SET jumlah_nilai = '$jumlah', kualitas_nilai = '$kualitas', waktu_nilai = '$waktu', biaya_nilai = '$biaya', penghitungan = '$hitung', nilai_capaian = '$capaian' , catatan = '$catat' where id = '$id'";
						echo $sqlx."<br>";
				 		mysql_query($sqlx);
					}else{
						$sqlx ="INSERT INTO dtl_penilaian_skp(nib_penilai,id_skp,no_tugas,jumlah_nilai,kualitas_nilai,waktu_nilai,biaya_nilai,catatan)
				...
				if ( empty($row_2) )
				{
				mysql_query("INSERT INTO mst_penilai(id,id_skp,nib_penilai,jabatan_penilai,tahun,tugas_tambahan_nilai_awal,kreativitas_nilai_awal,nilai_skp_awal)
								VALUES ('','$id_skp','$nib_penilai','$row[jabatan_atasan]','$row[tahun]','$tugas_tambahan_nilai_awal','$kreativitas_nilai_awal','$nilai_skp_awal')");
				}else{
				...
				if ( $status_real_awal == '2' )  $tgl_real_awal = date('Y-m-d');
				else $tgl_real_awal = $row['tgl_real_awal'];
				
				 mysql_query("UPDATE mst_skp SET status_real_awal = '$status_real_awal', tgl_real_awal = '$tgl_real_awal'
				 					 where id = '$id_skp'");
				?>
	...
	<form action="index.php?p=420&id_skp=<?php echo $id_skp ?>&nib_penilai=<?php echo $nib_penilai ?>&pagess=<?php echo $pagess ?>&cari=<?php echo $cari ?>" method="post" name="form" id="form">
	<table width="775" cellspacing="1" class="admintable">
		...
		<tr>
		  <td colspan="4" align="center" class="row7"><strong>Penilaian Kemajuan Capaian SKP Detil </strong></td>
	  </tr>
		<tr>
		  <td colspan="4" align="center" class="row7"><strong>Dari tanggal <?php echo reformat_tanggal($row['tgl_kemajuan_awal']) ?>  s/d tanggal  <?php echo reformat_tanggal($row['tgl_kemajuan_akhir']) ?></td>
	  </tr>
		<tr>
	...
	?>
				...
				<tr class="<?php echo $class ?>">
				  ...
				  <td align="left" valign="top"><?php echo $skp_dtl['nama_tugas'] ?>
                    ...
                    <td align="left" valign="top"><input name="jumlah_nilai[]" type="text" id="jumlah_nilai" size="5" value="<?php echo nilai_jumlah($nib_penilai,$id_skp,$skp_dtl['no_tugas']) ?>" onfocus="startCalculate(<?php echo $no-1;?>)" onblur="stopCalc()" <?php if ($row['status_real_awal'] == 0) echo "disabled=\"disabled\""; ?> />
                  ...
                  <td align="left" valign="top"><input name="kualitas_nilai[]" type="text" id="kualitas_nilai" size="5" value="<?php echo nilai_kualitas($nib_penilai,$id_skp,$skp_dtl['no_tugas']) ?>" onfocus="startCalculate(<?php echo $no-1;?>)" onblur="stopCalc()" <?php if ($row['status_real_awal'] == 0) echo "disabled=\"disabled\""; ?> />		     <input name="noxy[]" id="noxy" type="hidden" size="5" value="<?php echo $skp_dtl['kualitas_target'] ?>"/></td>
                    <td align="left" valign="top"><input name="waktu_nilai[]" type="text" id="waktu_nilai" size="5" value="<?php echo nilai_waktu($nib_penilai,$id_skp,$skp_dtl['no_tugas']) ?>" onfocus="startCalculate(<?php echo $no-1;?>)" onblur="stopCalc()" <?php if ($row['status_real_awal'] == 0) echo "disabled=\"disabled\""; ?> />
...
<!-- ajie -->				 
                 <?php 
	$datax = mysql_query("SELECT SUM(nilai_capaian) AS total_capaian FROM dtl_penilaian_skp where id_skp ='$id_skp' and nib_penilai = '$xusername_sess' ");
	
		
		$nx = mysql_query("select COUNT(*) as tot from dtl_penilaian_skp where id_skp = '$id_skp' and nib_penilai = '$xusername_sess' ");
		...
		$z = $x / $y;
				 ?>
                  ...
                  <input name="nilai_skp" type="text" size="5" value="<?php echo number_format(($z +$List_tambahan['tugas_tambahan_nilai_awal']+$List_tambahan['kreativitas_nilai_awal']),"2",".",",") ?>"/>
	...
	<tfoot>
		<tr>
			...
			<input name="status_real_awal" type="radio" value="0" <?php if( $row['status_real_awal'] == 0 ) echo 'checked="checked"' ?>/>&nbsp;&nbsp;Draf&nbsp;&nbsp;
	  		<input name="status_real_awal" type="radio" value="1" <?php if( $row['status_real_awal'] == 1 ) echo 'checked="checked"' ?> />&nbsp;&nbsp;Dikirim Pegawai&nbsp;&nbsp;
	  		<input name="status_real_awal" type="radio" value="2" <?php if( $row['status_real_awal'] == 2 ) echo 'checked="checked"' ?> />&nbsp;&nbsp;Dinilai Atasan&nbsp;&nbsp;			</td>
		    </tr>
	</tfoot>
</table>		  </td>
	    </tr>
...
function nilai_capaian($nib_penilai,$id_skp,$no_tugas) {
	...
	function total_nilai_capaian($nib_penilai,$id_skp) {
		...
		$data = mysql_query("SELECT SUM(nilai_capaian) AS total_capaian FROM dtl_penilaian_skp where id_skp ='$id_skp' and nib_penilai = '$nib_penilai' ");
		$rdata = mysql_fetch_array($data);
		$data = $rdata['total_capaian']; 
		$n = mysql_query("select COUNT(*) as n from dtl_penilaian_skp where id_skp = '$id_skp' and nib_penilai = '$nib_penilai'");
...
//ajie 16 juli	
	function catat($nib_penilai,$id_skp,$no_tugas) {
		$data = mysql_query("select catatan from dtl_penilaian_skp where nib_penilai = '$nib_penilai' and id_skp = '$id_skp' and no_tugas = '$no_tugas'");
		$rdata = mysql_fetch_array($data);
