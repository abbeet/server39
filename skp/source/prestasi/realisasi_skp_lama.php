<?php
	...
	$field = array("id","tahun","nib","kdunitkerja","kdjabatan","kdgol","is_approved_awal","tgl_approved_awal", "is_approved_akhir", "tgl_approved_akhir","nib_atasan", "jabatan_atasan", "kdgol_atasan","grade","status_real_akhir","tgl_real_akhir");
	//$field = get_field($table);
	$ed_link = "index.php?p=".get_edit_link($p);
	$del_link = "index.php?p=";
//	$kdunitkerja = '3400' ;
...
switch ($xlevel)
	...
	$query = "SELECT COUNT(*) as num FROM $table WHERE tahun = '$th' and is_approved_akhir = '1' and left(kdunitkerja,2) = '$xkdunit' and (nib LIKE '%$cari%' OR kdunitkerja LIKE '%$cari%' OR kdjabatan LIKE '%$cari%') ";
			break;
		default:
	$query = "SELECT COUNT(*) as num FROM $table WHERE tahun = '$th' and is_approved_akhir = '1' and nib = '$xusername' and (nib LIKE '%$cari%' OR kdunitkerja LIKE '%$cari%' OR kdjabatan LIKE '%$cari%') ";
			break;
	} 
...
switch ($xlevel)
	{
		case '1':
	$oList = mysql_query("SELECT * FROM $table WHERE tahun = '$th' and is_approved_akhir = '1' and (nib LIKE '%$cari%' OR kdunitkerja LIKE '%$cari%' OR kdjabatan LIKE '%$cari%') ORDER BY nib_atasan LIMIT $start, $limit");
			break;
		case '2':
	$oList = mysql_query("SELECT * FROM $table WHERE tahun = '$th' and is_approved_akhir = '1' and left(kdunitkerja,2) = '$xkdunit' and (nib LIKE '%$cari%' OR kdunitkerja LIKE '%$cari%' OR kdjabatan LIKE '%$cari%') ORDER BY kdunitkerja, nib LIMIT $start, $limit");
		default:
	$oList = mysql_query("SELECT * FROM $table WHERE tahun = '$th' and is_approved_akhir = '1' and nib = '$xusername' and (nib LIKE '%$cari%' OR kdunitkerja LIKE '%$cari%' OR kdjabatan LIKE '%$cari%') ORDER BY kdunitkerja, nib LIMIT $start, $limit");
			break;
	} 
			...
			foreach ($col[0] as $k=>$val) {
				...
				<tr class="<?php echo $class ?>">
				  ...
				  <?php switch ( $col[14][$k] )
				  		 { 
						      case '0' :?>
			  <a href="index.php?p=452&id_skp=<?php echo $col[0][$k] ?>&pagess=<?php echo $pagess ?>&cari=<?php echo $cari ?>" title="Edit Realisasi SKP">
			  ...
			  <?php     break;
				  ...
				  <td align="center" valign="top">
				  <?php if ( $col[14][$k] == '0' ) { ?>
				  <a href="index.php?p=453&id_skp=<?php echo $col[0][$k] ?>&pagess=<?php echo $pagess ?>&cari=<?php echo $cari ?>" title="Input Tugas Tambahan">
			  	  <img src="css/images/edit_f2.png" border="0" width="16" height="16"><font size="1">Tugas Tambahan</font></a>
			  	  <?php } ?>
				  </td>
				  <td align="center" valign="top">
				  <?php if ( $col[14][$k] == '0' ) { ?>
				  <a href="index.php?p=455&id_skp=<?php echo $col[0][$k] ?>&pagess=<?php echo $pagess ?>&cari=<?php echo $cari ?>" title="Input Kegiatan Kreativitas">
			     <img src="css/images/edit_f2.png" border="0" width="16" height="16"><font size="1">Kreatifitas</font></a>
			  	  <?php } ?>
				  </td>
				  <td align="center" valign="top">
			  <a href="source/prestasi/realisasi_skplama_prn.php?id_skp=<?php echo $col[0][$k] ?>" title="Cetak Realisasi SKP" target="_blank">
