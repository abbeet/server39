<?php	
	...
	$pdf->SetX($margin+$w[0]);
	$pdf->Cell($w[1],$ln,'PENILAIAN KEMAJUAN SASARAN KERJA','',1,'C');
	$pdf->SetX($margin+$w[0]);
	...
	$no += 1 ;
	$perhitungan = nilai_perhitungan($nib_penilai,$id_skp,$List['no_tugas']) ;
	...
	$pdf->Line($margin, $pdf->GetY(), $margin+$w2[0]+$w2[1]+$w2[2]*2+$w2[3]*2+$w2[4]*2+$w2[5]*2+$w2[6]*2+$w2[7]+$w2[8],$pdf->GetY());
		...
		$pdf->SetX($margin+$w2[0]+$w2[1]+$w2[2]*2+$w2[3]*2+$w2[4]*2+$w2[5]*2+$w2[6]*2+$w2[7]);
		$pdf->Cell($w2[8],$ln,number_format($row_penilai['nilai_skp_awal'],"2",",","."),'LR',1,'R');
		$pdf->SetX($margin);
