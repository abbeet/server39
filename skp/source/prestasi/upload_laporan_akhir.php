<?php
	checkauthentication();
	$table = "dtl_skp";
	$field = array("id","no_tugas","nama_tugas","jumlah_target","satuan_jumlah","jumlah_real","kualitas_real","id_skp");
	$err = false;
	$p = $_GET['p'];
	$q = $_GET['q'];
	$th = $_SESSION['xth'];
		...
		if ($count == 0) { ?>
			...
			foreach ($col[0] as $k=>$val) {
				...
				<tr class="<?php echo $class ?>">
					...
					<?php file_laporan($col[7][$k],$col[1][$k]) ?>
					</td>
			      <td width="6%" align="center" valign="top"><?php if ( $col[5][$k] <> 0 ) { ?><a href="index.php?p=426&id=<?php echo $col[0][$k]; ?>&id_skp=<?php echo $col[7][$k]; ?>">upload</a><?php } ?></tr>
