	...
	if (isset($form)) {		
		if ($err != true) {
			...
			if ($q == "") {
				...
				<meta http-equiv="refresh" content="0;URL=index.php?p=264&id_skp=<?php echo $id_skp ?>&pagess=<?php echo $pagess ?>&cari=<?php echo $cari ?>&sw=<?php echo $sw ?>&xkdunit=<?php echo $_REQUEST['xkdunit'] ?>"><?php
				exit();
			} 
			else {
				// UPDATE
...
<body onLoad="showText(0)">
	...
	<form action="index.php?p=264&id_skp=<?php echo $id_skp ?>&pagess=<?php echo $pagess ?>&cari=<?php echo $cari ?>&sw=<?php echo $sw ?>&xkdunit=<?php echo $_REQUEST['xkdunit'] ?>" method="post" name="form">
	<table width="755" cellspacing="1" class="admintable">
		...
		<tr>
		  ...
		  <td width="207"><?php echo nama_peg($row['nib_atasan']) ?>
	      <input type="hidden" name="<?php echo $field[10] ?>" size="20" value="<?php echo $row['id'] ?>" /></td>
		...
		<tr>
		    ...
		    <td class="key">Jabatan</td>
		    <td>
			<?php echo nm_jabatan_ij($row['kdjabatan']) ?></td>
	    </tr>
		<tr>
		...
		<tr>
			...
			<td colspan="3">			
				<div class="button2-right">
					<div class="prev">
					<?php if ( $sw == '' ) {?>
						<a onClick="Cancel('index.php?p=<?php echo $p_next ?>&pagess=<?php echo $pagess ?>&cari=<?php echo $cari ?>&xkdunit=<?php echo $_REQUEST['xkdunit'] ?>')">Kembali</a>		
					<?php }else{ ?>	
						<a onClick="Cancel('index.php?p=252&pagess=<?php echo $pagess ?>&cari=<?php echo $cari ?>&xkdunit=<?php echo $_REQUEST['xkdunit'] ?>')">Kembali</a>		
					<?php } ?>
				  </div>
				</div>
