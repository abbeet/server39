...
?>		
	...
	?>
				...
				<tr class="<?php echo $class ?>">
                  ...
                  <!--ajie 18 june-->
					<input name="nilai_capaian[]" id="nilai_capaian" type="text" size="5" value="<?php echo number_format($perhitungan/3,"2",".",",") ?>" onFocus="startCalculate(<?php echo $no-1;?>)" onBlur="stopCalc()"/></td>
				</tr>
...
if (l<=24 ) {
	...
	m = (((1.76 * parseInt(k,10)) - parseInt(d,10)) / parseInt(k,10)) * 100 ;
		...
		document.form.nilai_capaian[i].value = 	((parseInt(a,10)/parseInt(e,10)*100 ) + (parseInt(b,10)/parseInt(f,10)*100 ) + j + m)/3;
