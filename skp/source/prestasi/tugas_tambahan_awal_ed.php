<?php
	checkauthentication();
	$table = "dtl_skp_tugas_tambahan";
	$field = array("id","id_skp","no_tugas","nama_tugas");
	$err = false;
	$p = $_GET['p'];
	$q = $_GET['q'];
	...
	if (isset($form)) {		
		if ($err != true) {
			...
			if ($q == "") {
				...
				<meta http-equiv="refresh" content="0;URL=index.php?p=412&id_skp=<?php echo $id_skp ?>&pagess=<?php echo $pagess ?>&cari=<?php echo $cari ?>&sw=<?php echo $sw ?>"><?php
				exit();
			} 
			else {
				// UPDATE
	...
	<form action="index.php?p=412&id_skp=<?php echo $id_skp ?>&pagess=<?php echo $pagess ?>&cari=<?php echo $cari ?>&sw=<?php echo $sw ?>" method="post" name="form">
	<table width="755" cellspacing="1" class="admintable">
		...
		<tr>
		  <td colspan="4" align="center" class="row7"><strong>Tugas Tambahan SKP Detil </strong></td>
	  </tr>
		<tr>
		  <td colspan="4" align="center" class="row7"><strong>Penilaian dari tanggal <?php echo reformat_tanggal($row['tgl_kemajuan_awal']) ?> s/d <?php echo reformat_tanggal($row['tgl_kemajuan_akhir']) ?></strong></td>
	  </tr>
		<tr>
		  <td class="key">Lihat Akitivitas Harian</td>
		  <td colspan="3"><b><a href="index.php?p=414&id_skp=<?php echo $id_skp; ?>&pagess=<?php echo $_GET['pagess']; ?>&cari=<?php echo $_GET['cari']; ?>">Tugas Tambahan</a></b></td>
		</tr>
		<tr>
		  <td class="key">No Urut Tugas Tambahan</td>
		  <td colspan="3"><input type="text" name="<?php echo $field[2] ?>" size="3" value="<?php echo $value[2] ?>" /></td>
	  </tr>
		
		<tr>
		  <td class="key">Kegiatan Tugas Tambahan </td>
...
<table width="58%" cellpadding="1" class="adminlist">
	...
	<tbody><?php
		if ($count == 0) { ?>
		  ...
		  <a href="index.php?p=412&q=<?php echo $col[0][$k] ?>&id_skp=<?php echo $id_skp ?>&pagess=<?php echo $pagess ?>&cari=<?php echo $cari ?>&sw=<?php echo $sw ?>" title="Edit">
