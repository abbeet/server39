<?php	
	...
	$status = $_POST['kode'];
	$sw = $_REQUEST['sw'];  # 0 untuk approve pegawai  1 untuk approv atasan
	$tgl = date ('Y-m-d') ;
//----- Kirim pegawai
// $status == 1
// $sw = 0	
...
*/
	...
	$pdf->SetX($margin+$w[0]);
	$pdf->Cell($w[1],$ln,'TAHUN '.date("Y"),'',1,'C');
	$pdf->Ln()*2;
	...
	$arrField_1	  = $pdf->SplitToArray($w1[2],$ln,reformat_nipbaru(nip_peg($row['nib_atasan'])));
	...
	$arrField_1	  = $pdf->SplitToArray($w1[2],$ln,nm_pangkat($row['kdgol_atasan']).' ('.nm_gol($row['kdgol_atasan']).')');
	...
	if ( kdunitkerja_peg($row['nib_atasan']) <> '0000' )   $arrField_1	  = $pdf->SplitToArray($w1[2],$ln,trim(skt_unitkerja(substr(kdunitkerja_peg($row['nib_atasan']),0,2))).' - BATAN');
	else  $arrField_1	  = $pdf->SplitToArray($w1[2],$ln,'BATAN');  
	...
	for($i=0;$i<$max;$i++)
	{
		...
		$pdf->SetX($margin+$w2[0]+$w2[1]+$w2[2]);
		$pdf->Cell($w2[3],$ln,$arrOutput[$i],'LR',0,'L');
		$pdf->SetX($margin+$w2[0]+$w2[1]+$w2[2]+$w2[3]);
		$pdf->Cell($w2[4],$ln,$arrMutu[$i],'LR',0,'C');
		$pdf->SetX($margin+$w2[0]+$w2[1]+$w2[2]+$w2[3]+$w2[4]);
		$pdf->Cell($w2[5],$ln,$arrWaktu[$i],'LR',0,'L');
...
//------------------ Tanda Tangan	
	...
	$pdf->SetX($margin);
	$pdf->Cell($w3[0],$ln,'NIP. '.reformat_nipbaru(nip_peg($row['nib_atasan'])),'',0,'C');
	$pdf->SetX($margin+$w3[0]+$w3[1]);
