<?php	
	...
	$pdf->SetX($margin+$w[0]);
	$pdf->Cell($w[1],$ln,'REALISASI SASARAN KERJA','',1,'C');
	$pdf->SetX($margin+$w[0]);
	...
	$arrAk_kredit_r  = $pdf->SplitToArray($w2[2],$ln,number_format($List['ak_real'],"4",",","."));
	$arrOutput_r	 = $pdf->SplitToArray($w2[3],$ln,$List['jumlah_real'].' '.trim($List['satuan_jumlah']));
	$arrMutu_r	 	 = $pdf->SplitToArray($w2[4],$ln,$List['kualitas_real'].' %');
	$arrWaktu_r	 	 = $pdf->SplitToArray($w2[5],$ln,$List['waktu_real'].' '.trim($List['satuan_waktu']));
	$arrBiaya_r	  	 = $pdf->SplitToArray($w2[6],$ln,number_format($List['biaya_real'],"0",",","."));
