<?php
	checkauthentication();
	$table = "dtl_skp";
	$field = array("id","no_tugas","nama_tugas","jumlah_target","satuan_jumlah","jumlah_real_awal","kualitas_real_awal","id_skp");
	$err = false;
	$p = $_GET['p'];
	$q = $_GET['q'];
	$th = $_SESSION['xth'];
		...
		if ($count == 0) { ?>
			...
			foreach ($col[0] as $k=>$val) {
				...
				<tr class="<?php echo $class ?>">
					...
					<td align="center" valign="top"><?php if ( $col[5][$k] <> 0 ) { ?><?php echo $col[5][$k].' '.$col[4][$k] ?><?php } ?></td>
				    <td align="center" valign="top"><?php if ( $col[5][$k] <> 0 ) { ?><?php echo $col[6][$k].' %' ?><?php } ?></td>
				    <td align="center" valign="top"><?php echo $col[0][$k]?></td>
			      <td width="6%" align="center" valign="top"><a href="index.php?p=426&id=<?php echo $col[0][$k]; ?>&id_skp=<?php echo $col[7][$k]; ?>&no_tugas=<?php echo $col[1][$k]; ?>&nama_tugas=<?php echo $col[2][$k]; ?>&nib=<?php echo $row['nib']; ?>">upload</a></tr>
