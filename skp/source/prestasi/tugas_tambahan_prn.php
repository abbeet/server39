...
//		}		
	...
	$w = array(50,5,80);
	$pdf->SetX($margin+$w[0]);
	$pdf->Cell($w[2],$ln*6,'','',1,'C');
	$pdf->SetX($margin+$w[0]);
	$pdf->Cell($w[2],$ln,'SURAT KETERANGAN','',1,'C');
	$pdf->SetX($margin+$w[0]);
	$pdf->Cell($w[2],$ln,'MELAKSANAKAN TUGAS TAMBAHAN','',1,'C');
	$pdf->SetX($margin+$w[0]);
...
if( substr($row['kdunitkerja'],1,3) == '000' and substr($row['kdjabatan'],0,4) == '0011' ) 
	...
	$arrUraian		= $pdf->SplitToArray($w[1],$ln,'Telah melaksanakan :');
	...
	$max = 0 ;
	$arrNomor		= $pdf->SplitToArray($w[1],$ln,'a.');
	$arrLabel		= $pdf->SplitToArray($w[2],$ln,'Tugas tambahan sebagai :');
	...
	for($i=0;$i<$max;$i++)
	{
		...
		$data_tugas = mysql_query("select tugas_tambahan_nilai_awal from mst_penilai where id_skp = '$id_skp' ");
		while ( $rdata_tugas = mysql_fetch_array($data_tugas) ) {
		
		if ( $rdata_tugas['tugas_tambahan_nilai_awal'] <> 0 )  
		{
			 $jml_tugas += 1 ;
		     $nilai_tugas += $rdata_tugas['tugas_tambahan_nilai_awal'];
		}
		
		}
		if ( $jml_tugas <> 0 )     $nilai_tugas = $nilai_tugas/$jml_tugas ;
		else    $nilai_tugas = 0 ;
	...
	$no += 1 ;
	$max = 0 ;
	$arrNomor		= $pdf->SplitToArray($w[2],$ln,$List['no_tugas'].'.');
	$arrUraian		= $pdf->SplitToArray($w[3],$ln,$List['nama_tugas']);
	...
	$max = 0 ;
	$arrNomor		= $pdf->SplitToArray($w[1],$ln,'b.');
	$arrLabel		= $pdf->SplitToArray($w[2],$ln,'Jumlah tugas tambahan selama 1 tahun, diberikan nilai '.$nilai_tugas);
