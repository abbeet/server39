<?php
	...
	$query = "SELECT COUNT(*) as num FROM $table order by (tahun + bulan) ";
	$total_pages = mysql_fetch_array(mysql_query($query));
	$total_pages = $total_pages[num];
	
	#variabel query
	$targetpage = "index.php?p=$p&cari=$cari"; 	#nama file  (nama file ini)
	$limit = 13; 							#berapa yang akan ditampilkan setiap halaman
	$pagess = $_GET['pagess'];
	if($pagess) 
	...
	$oList = mysql_query("SELECT * FROM $table ORDER BY (tahun*12) + bulan LIMIT $start, $limit");
	...
	echo $pagination;
?>
<table width="31%" cellpadding="1" class="adminlist">
	<thead>
		
		<tr>
		  <th width="6%">No.</th>
		  <th width="35%">Bulan</th>
		  <th width="9%">Jumlah Hari Efektif </th>
		...
		if ($count == 0) { ?>
			...
			foreach ($col[0] as $k=>$val) {
				...
				<tr class="<?php echo $class ?>">
					...
					<td align="left" valign="top"><?php echo nama_bulan($col[2][$k]).' '.$col[1][$k] ?></td>
