<?php
	...
	$query = "SELECT COUNT(*) as num FROM $table WHERE left(nmjabatan,12) <> 'Kepala DINAS' AND ( KdKel LIKE '%$cari%' OR KdJab LIKE '%$cari%' OR NmJabatan LIKE '%$cari%' ) order by kode";
	...
	$oList = mysql_query("SELECT * FROM $table WHERE left(nmjabatan,12) <> 'Kepala DINAS' AND ( KdKel LIKE '%$cari%' OR KdJab LIKE '%$cari%' OR NmJabatan LIKE '%$cari%') ORDER BY kdunitkerja,klsjabatan desc,kdkel,kdjab,nmjabatan LIMIT $start, $limit");
...
<div align="right">
	<form action="" method="get">
		...
		<a href="index.php?p=236">Reset</a>
	</form>
</div><br />
