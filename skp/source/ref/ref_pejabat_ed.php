<?php
	...
	{		
		...
		{	
			...
			{
				//ADD NEW				
				$kdpejabat = $_REQUEST['kdpejabat'];
				$nib = $_REQUEST['nib'];
				$sql = "INSERT INTO $table (id, th, kdsatker, kdpejabat, nib) VALUE ('','$th','$kdsatker','$kdpejabat','$nib')";
				$rs = mysql_query($sql);
	...
	<table width="671" cellspacing="1" class="admintable">
		...
		<tr>
		  ...
		  <td><select name="kdpejabat">
              <option value="<?php echo $value['kdpejabat'] ?>"><?php echo nm_jabatan_keu($value['kdpejabat']) ?></option>
		...
		<tr>
		  ...
		  <td><select name="nib">
              <option value="<?php echo $value['nib'] ?>"><?php echo $value['nib'].' '.nama_peg($value['nib']) ?></option>
			  <?php
					...
					{ ?>
              <option value="<?php echo $Pegawai['nib'] ?>"><?php echo $Pegawai['nib'].' '.nama_peg($Pegawai['nib']) ?></option>
			  <?php
					} ?>
            </select></td>
	  </tr>
		...
		{ ?>
			...
			foreach ($col[0] as $k=>$val) {
				...
				<tr class="<?php echo $class ?>">
					...
					<td align="center">
						<a href="index.php?p=360&kdsatker=<?php echo $kdsatker; ?>&q=<?php echo $col[0][$k]; ?>" title="Edit">
