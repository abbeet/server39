...
function nama_hari($ymd) {
	...
	function rp_grade($kdgrade) {
	   
		$data = mysql_query("select rp_grade from kd_grade where kd_grade = '$kdgrade'");
		$rdata = mysql_fetch_array($data);
		$result = $rdata['rp_grade'];
		return $result;
	}
	...
	function nm_jabatan_ij($kode,$kdunit) {
		$kdunit = substr($kdunit,0,5) ;
		$data = mysql_query("select kode,nmjabatan from kd_jabatan where kode='$kode' and kdunitkerja LIKE '$kdunit%'");
		$rdata = mysql_fetch_array($data);
		$result = trim($rdata['nmjabatan']);
		return $result;
	}
	...
	function grade_jabatan_ij($kode) {
		$kdunit = substr($kdunit,5,2) ;  
		$data = mysql_query("select klsjabatan from kd_jabatan where kode='$kode' and kdunitkerja LIKE '$kdunit%'");
		$rdata = mysql_fetch_array($data);
		$result = trim($rdata['klsjabatan']);
		return $result;
	}
	...
	function nama_peg($kode) {
		$data = mysql_query("select nama from m_idpegawai where nip='$kode'");
		$rdata = mysql_fetch_array($data);
		$result = trim($rdata['nama']);
		return $result;
	}
	...
	function kdeselon_peg($kode) {
		$data = mysql_query("select kdeselon from m_idpegawai where nip='$kode'");
		$rdata = mysql_fetch_array($data);
	...
	function kdjabatan_peg($kode) {
		$data = mysql_query("select KdKelJabatan,KdFungsional from m_idpegawai where Nib='$kode'");
		$rdata = mysql_fetch_array($data);
		$result = $rdata['KdKelJabatan'].$rdata['KdFungsional'];
		return $result;
	}

	function jabatan_peg($th,$kode) {
		$data = mysql_query("select kdjabatan from mst_skp where nip = '$kode' and tahun = '$th'");
		$rdata = mysql_fetch_array($data);
		$result = nm_jabatan_ij($rdata['kdjabatan']);
		return $result;
	}

	function jabstruk_peg($kode) {
		$kd = '001'.substr($kode,2,4);
		$data = mysql_query("select nmjabatan from kd_jabatan where kode = '$kd'");
		$rdata = mysql_fetch_array($data);
		$result = $rdata['nmjabatan']; 
		return $result;
	}

	function jabfung_peg($kode) {
		$data = mysql_query("select KdKelJabatan,KdFungsional from m_idpegawai where Nib='$kode'");
		$rdata = mysql_fetch_array($data);
		$kdkeljabatan = $rdata['KdKelJabatan'];
		$kdfungsional = $rdata['KdFungsional'];
			if ( $kdkeljabatan <> '' and $kdfungsional <> ''){
			 	$result =  nm_jabfung($kdkeljabatan,$kdfungsional) ;
			}else{
			    $result = '';
	...
	function kdunitkerja_nip($kode) {
		$data = mysql_query("select kdunitkerja from m_idpegawai where nip='$kode' ");
		$rdata = mysql_fetch_array($data);
		$result = $rdata['kdunitkerja'];
		return $result;
	}
	
	function kdunitkerja_peg($th,$kode) {
		$data = mysql_query("select kdunitkerja from mst_skp where nip='$kode' and tahun = '$th' ");
		$rdata = mysql_fetch_array($data);
		$result = $rdata['kdunitkerja'];
		return $result;
	}
	...
	function nip_peg_j2($th,$kdunit,$kdjabatan) {
		$data = mysql_query("select nib from mst_skp where kdunitkerja = '$kdunit' and kdjabatan = '$kdjabatan' and tahun = '$th'");
		$rdata = mysql_fetch_array($data);
		$result = $rdata['nib'];
		return $result;
	}

	function kdgol_peg($th,$kode) {
		$data = mysql_query("select kdgol from mst_skp where nip='$kode' and tahun = '$th'");
		$rdata = mysql_fetch_array($data);
		$result = $rdata['kdgol'];
		return $result;
	}
	
	function kdgol_nip($kode) {
		$data = mysql_query("select kdgol from m_idpegawai where nip='$kode'");
		$rdata = mysql_fetch_array($data);
		$result = $rdata['kdgol'];
		return $result;
	}
	
	function kdjnsskfung_peg($kode) {
		$data = mysql_query("select KdJnsSKFung from m_idpegawai where Nib='$kode'");
		$rdata = mysql_fetch_array($data);
		$result = $rdata['KdJnsSKFung'];
		return $result;
	}
	
	function tmtgol_peg($kode) {
		$data = mysql_query("select TMTGol from m_idpegawai where Nib='$kode'");
		$rdata = mysql_fetch_array($data);
		$result = $rdata['TMTGol'];
		return $result;
	}

	function nilai_perhitungan_akhir($nib_penilai,$id_skp,$no_tugas) {
		$data_nilai = mysql_query("select jumlah_nilai,kualitas_nilai,waktu_nilai from dtl_penilaian_skp_akhir where nib_penilai = '$nib_penilai' and id_skp = '$id_skp' and no_tugas = '$no_tugas'");
...
if ( $rdata_target['waktu_target'] <> 0 )
{
		$efisiensi_waktu = 100 - ( $rdata_nilai['waktu_nilai']/$rdata_target['waktu_target'] * 100 ) ;
		if ( $efisiensi_waktu <= 24 )
		{
			$penilaian_waktu = ((( 1.76 * $rdata_target['waktu_target']) - $rdata_nilai['waktu_nilai']) / $rdata_target['waktu_target'])*100 ;
		}else{
			$penilaian_waktu = 76 - ( ( ((( 1.76 * $rdata_target['waktu_target']) - $rdata_nilai['waktu_nilai']) / $rdata_target['waktu_target'])*100 ) - 100 ) ;
		}
}else{
			$penilaian_waktu = 0 ;
}

if ( $rdata_target['biaya_target'] <> 0 )
{
		$efisiensi_biaya = 100 - ( $rdata_nilai['biaya_nilai']/$rdata_target['biaya_target'] * 100 ) ;
		if ( $efisiensi_biaya <= 24 )
		{
			$penilaian_biaya = ((( 1.76 * $rdata_target['biaya_target']) - $rdata_nilai['biaya_nilai']) / $rdata_target['biaya_target'])*100 ;
		}else{
			$penilaian_biaya = 76 - ( ( ((( 1.76 * $rdata_target['biaya_target']) - $rdata_nilai['biaya_nilai']) / $rdata_target['biaya_target'])*100 ) - 100 ) ;
		}
}else{
		$penilaian_biaya = 0 ;
}
		$result = ($rdata_nilai['jumlah_nilai']/$rdata_target['jumlah_target'])*100 + 
				  ($rdata_nilai['kualitas_nilai']/$rdata_target['kualitas_target'])*100 + $penilaian_waktu + $penilaian_biaya
...
if ( $rdata_target['biaya_target'] <> 0 )
{
		...
		$penghitungan = ($rdata_nilai['jumlah_nilai']/$jumlah_target)*100 + 
				  ($rdata_nilai['kualitas_nilai']/$kualitas_target)*100 + $penilaian_waktu + $penilaian_biaya ;
		$capaian += $penghitungan/4 ;		  
}else{
		$penilaian_biaya = 0 ;
		$penghitungan = ($rdata_nilai['jumlah_nilai']/$jumlah_target)*100 + 
				  ($rdata_nilai['kualitas_nilai']/$kualitas_target)*100 + $penilaian_waktu ;
		$capaian += $penghitungan/3 ;
}
		}
		if ( $no == 0 )  $result = 0 ;
		if ( $no <> 0 )  $result = $capaian/$no ;
		return $result;
	}
	...
	function nama_perilaku($kode) {
		if ($kode <= 4 and $kode > 3 ) 	$hasil = 'Sangat Baik';
		if ($kode <= 3 and $kode > 2 ) 	$hasil = 'Baik';
		if ($kode <= 2 and $kode > 1 ) 	$hasil = 'Cukup';
		if ($kode <= 1 and $kode > 0 ) 	$hasil = 'Kurang';
		return $hasil;
	}
...
*/
	...
	function tunker_mst_tk($nib,$th,$kdbulan,$grade,$kdunitkerja) {
		$data = mysql_query("select tunker from mst_tk where nib = '$nib' and tahun = '$th' and bulan = '$kdbulan' and grade = '$grade' and kdunitkerja = '$kdunitkerja' ");
		$rdata = mysql_fetch_array($data);
		$result = $rdata['tunker'];
		return $result;
	}
	...
	function jmlpeg_bulan_grade($th,$kdbulan,$kdunit,$grade) {
		$result = 0 ;
		$xkdunit = substr($kdunit,0,5) ;
		if ( $kdunit == '2320100' )
		{
			$data = mysql_query("select count(nip) as  jumlah from mst_tk where tahun = '$th' and bulan = '$kdbulan' and ( kdunitkerja LIKE '$xkdunit%' OR kdunitkerja = '2320000' ) and grade = '$grade' group by grade");
		}else{
			$data = mysql_query("select count(nip) as  jumlah from mst_tk where tahun = '$th' and bulan = '$kdbulan' and kdunitkerja LIKE '$xkdunit%' and grade = '$grade' group by grade");
		}
		$rdata = mysql_fetch_array($data);
		$result = $rdata['jumlah'] ;
		return $result;
	...
	function jmlpeg_bulan_grade_ditjen($th,$kdbulan,$grade) {
		$result = 0 ;
		$data = mysql_query("select count(nip) as  jumlah from mst_tk where tahun = '$th' and bulan = '$kdbulan' and grade = '$grade' group by grade");
		$rdata = mysql_fetch_array($data);
		$result = $rdata['jumlah'] ;
		return $result;
	}
