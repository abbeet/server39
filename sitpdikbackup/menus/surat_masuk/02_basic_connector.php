...
//	}
		function formatting($row){
			$suratmasukviewlink = 106;
                  //render field as details link
                  $data = $row->get_value("NoSurat");
                 // $row->set_value("TujuanSurat","<a href='details.php?id={$data}'><img src='image/ico_view.gif' alt='Lihat'> </a>");
				  $alamat = "index.php?p=" . enkripsi($suratmasukviewlink . "&q=".$data);
			
				  $row->set_value("IdPenerima","<a href='$alamat'><img src='images/ico_view.gif' > </a>");
				  
				  
				 
				    $data = $row->get_value("TglTerima");
                  $row->set_value("TglTerima",date("d-m-Y",strtotime($data)));
...
//	$grid->render_table("suratmasuk","IdSuratMasuk","TujuanSurat,NoSurat,AsalSurat,Perihal,TglTerima");
	$grid->render_sql("SELECT * FROM vcariperorang WHERE IdPenerima = '".$IdUser."' order by TglDisposisi desc","IdSuratMasuk","IdPenerima,NoSurat,AsalSurat,Perihal,TglDisposisi");
