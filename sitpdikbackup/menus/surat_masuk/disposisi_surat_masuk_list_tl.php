<?php
	...
	$disposisisuratmasukviewlink = 104;
	//$memoviewlink = 81;
	...
	if (@$_POST['c'])
	{
		$sql = sql_select("suratmasuk", "*", "DicatatOleh = '".$KdSatker."' AND Perihal LIKE '%".$c."%'", "TglTerima DESC");
	}
	
	else
	{
		$sql = sql_select("suratmasuk", "*", "DicatatOleh = '".$KdSatker."'", "TglTerima DESC");
	}
...
<?php
  ...
  ?>
            ...
            <th rowspan="2" width="9%" align="center"><strong>Status Tindak Lanjut</strong></font></th>
	...
	<tbody><?php
		...
		else
		{
			...
			while ($list = mysql_fetch_array($olist))
			{
				...
				$tl_status[$i]		= $list['tl_status'];
				$tglakhir[$i]		= $list['tglakhir'];	
			   ...
			   <td>
                ...
                <td align="center">
                <?php if ($tl_status[$i] == 1) echo "<font color='#0099FF'>Sudah</font>";
				else if ($tl_status[$i] == 2) echo "Tidak Perlu";
				else
				 echo "<font color='#FF0000'>Belum <br> ". $tglakhir[$i] ."</font>" ;
				 ?>
                  <?//=$tujuansurat[$i] ?>
               </td>
             <!--  <td class="tablebody">
