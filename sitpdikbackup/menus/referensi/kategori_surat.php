<?php
	...
	if (@$_POST['c'])
	{
		$sql = sql_select("kategorisurat", "*", "IdKategori LIKE '%".$c."%' OR NmKategori LIKE '%".$c."%'", "IdKategori");
	}
	
	else
	{
		$sql = sql_select("kategorisurat", "*", "", "IdKategori");
	}
...
<table class="adminlist" cellpadding="1">
	...
	<tbody><?php
		...
		else
		{
			...
			{
				$id[$i]		= $list['IdKategori'];
				$nama[$i]	= $list['NmKategori'];
