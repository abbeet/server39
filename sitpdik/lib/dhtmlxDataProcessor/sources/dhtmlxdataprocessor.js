	...
	*/
	...
	markRow:function(id,state,mode){ 
		...
		if (this.callEvent("onRowMark",[id,state,mode,invalid])){
			var method = (this.obj.mytype=="tree")?"setItemStyle":"setRowTextStyle";
			//default logic
			if (state)
				str+=this.styles[mode];
			else
        		str+=this.styles.clear;
        	this.obj[method](id,str);

			if (invalid && invalid.details){
				str+=this.styles[invalid+"_cell"];
				for (var i=0; i < invalid.details.length; i++)
					if (invalid.details[i])
        				this.obj.setCellTextStyle(id,i,str);
			}
		}
	},
		...
		if (this._tSend) 
			...
			for (var i=0; i<this.updatedRows.length; i++)
				if (!this._in_progress[this.updatedRows[i]]){
					...
					if (this._waitMode && this.obj.mytype=="tree") return; //block send all for tree
				}
	},
...
*/
	afterUpdateCallback:function(sid, tid, action, btag) {
	    ...
	    case "deleted":
	        ...
	        return this.callEvent("onAfterUpdate", [sid, action, tid])
	        break;
	    }
	    //???
	...
	*/
	afterUpdate:function(that,b,c,d,xml){
		...
		if (!that._tSend && that.obj.mytype=="tree" && that.updatedRows.length) 
			that.sendData();
