...
function GetMonthName($iMonth, $sType, $lang = "en") {
	if ($lang == "en") {
		if ($sType == "shortdate") {
			$MonthIndonesia = array ('01'=>'Jan', '02'=>'Feb', '03'=>'Mar', '04'=>'Apr', '05'=>'Mei', '06'=>'Jun', '07'=>'Jul', '08'=>'Aug', '09'=>'Sep', '10'=>'Oct', '11'=>'Nov', '12'=>'Dec');

			return $MonthIndonesia[$iMonth];
		} if ($sType == "longdate") {
			$MonthIndonesia = array ('01'=>'January', '02'=>'February', '03'=>'March', '04'=>'April', '05'=>'Mei', '06'=>'June', '07'=>'July', '08'=>'August', '09'=>'September', '10'=>'October', '11'=>'November', '12'=>'December');
...
http://www.zend.com/codex.php?id=947&single=1
*/
function ConvertMoneyFormat($string) {
 ...
 function hitung_user_online()
{
   ...
   if ($count < 1)
   {
      $query = "INSERT INTO user_online
                VALUES ('" .$session_id. "', '".$waktu_masuk."')";
   }
   else
   {
      $query = "UPDATE user_online
                SET timestamp='" . $waktu_masuk . "'  
                WHERE session_id='" . $session_id . "'";
   }
   $result = mysql_query($query);
   $query = "SELECT count(*) total FROM user_online where session_id <> '' ";
   $record = mysql_fetch_array(mysql_query($query));
   $user_online = $record['total'];
   $query = "DELETE FROM user_online
             WHERE timestamp < " . ($waktu_masuk - $waktu);
   mysql_query($query);
   return $user_online;
}
?>
