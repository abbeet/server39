<?php
	...
	if ($xlogin == "28B60A2D") 
	{
		if ($xusername != "")
		{
			if ($xpassword != "")
			{
				$sql = "SELECT username, nama, password, level, unit, aktif, reset, kunci FROM xuser_pegawai WHERE 
				username = '".$xusername."'";
				...
				if ($nuser == 1)
				{
					...
					if ($xuser['aktif'] == "1")
					{
						...
						if (decode_password($xuser['password'], $len) == md5($xpassword))
						{
							...
							$_SESSION['xnama_'.$session_name] 	= $xuser['nama'];
							...
							$_SESSION['xlevel_'.$session_name] = $xuser['level'];
