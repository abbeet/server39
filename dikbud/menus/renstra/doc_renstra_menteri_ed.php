...
//			if( $xkdunit <> '' ) {
			...
			if ($_FILES['nmfile']['name'] != "")
			{
				...
				$filedir = "file_renstra/480000/".$th.'_'.$filename ;
				move_uploaded_file($_FILES["nmfile"]["tmp_name"], $filedir);
				//-------- simpan data ------
				$tgl_upload = date ('Y-m-d H:i:s') ;
				$sql = mysql_query("select * from dt_fileupload WHERE th = '$th' and kdunitkerja = '480000' and nourut = '$nourut' and keterangan = 'Renstra' ");
				$row = mysql_fetch_array($sql);
				if(empty($row)){
					$sql = "INSERT INTO dt_fileupload (id,th,kdunitkerja,nourut,dafisi,nama_file,keterangan,tgl_upload,user_upload) values ('','$th','480000','$nourut','$dafisi','$nm_filename','Renstra', '$tgl_upload','$xusername')";				
					$query = mysql_query($sql) or die(mysql_error());
				}else{
					$sql = "UPDATE dt_fileupload SET dafisi = '$dafisi', nama_file = '$nm_filename', tgl_upload = '$tgl_upload', user_upload = '$xusername' WHERE th = '$th' and kdunitkerja = '480000' and nourut = '$nourut' and keterangan = 'Renstra' ";				
					$query = mysql_query($sql) or die(mysql_error());
				}
				//--------------------
			...
			<meta http-equiv="refresh" content="0;URL=index.php?p=445"><?php
			exit();
	...
	function form_proses()
		{
            ...
            <form action="index.php?p=445" method="post" name="xproses" enctype="multipart/form-data">
	
  <table cellspacing="1" class="admintable">
    <tr> 
      <td width="122" class="key">Tahun Anggaran</td>
      <td width="517"><?php echo $th ?></td>
    </tr>
...
<?php
	$sql = "select * from dt_fileupload WHERE th = '$th' and kdunitkerja = '480000' and keterangan = 'Renstra' order by nourut";
...
<table width="463" cellpadding="1" class="adminlist">
  ...
  <tbody>
    ...
    <tr class="<?php echo $class ?>">
      ...
      <td width="10%" align="center"><?php echo ket_unit($col[7][$k]) ?></td>
      <td width="11%" align="center"><?php echo $col[6][$k]?></td>
      <td width="29%" align="left"><?php echo $col[1][$k]?></td>
