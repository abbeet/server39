<?php
	...
	$sql = "select sum(TOTNILMAK) as real_giat from $table WHERE THANG='$th' AND KDSATKER='$kdsatker' and KDGIAT='$kdgiat' group by KDGIAT";
...
<table width="738" cellpadding="1" class="adminlist">
  <thead>
    <tr> 
      <th>Kode APBN</th>
      <th colspan="5"><font color="#0FA40B">Kegiatan</font> / <font color="#FF0000">Output</font> / Akun / <font color="#3333FF">SPM & SP2D</font></th>
      <th>Anggaran</th>
    </tr>
  </thead>
  <tbody>
    <tr class="<?php echo $class ?>"> 
      ...
      <td width="12%" align="right"><font color="#0FA40B"><strong><?php echo number_format($Giat['real_giat'],"0",",",".") ?></strong></font></td>
    </tr>
    <?php 
	$sql = "select KDOUTPUT, sum(TOTNILMAK) as real_output from $table WHERE THANG='$th' and KDSATKER='$kdsatker' and KDGIAT='$kdgiat' group by KDOUTPUT order by KDOUTPUT";
...
?>
	...
	$sql = "select NOSPM,TGSPM,NOSP2D,TGSP2D, sum(TOTNILMAK) as real_spm from $table WHERE THANG='$th' and KDSATKER='$kdsatker' and KDOUTPUT='$Output[KDOUTPUT]' group by concat(NOSPM,NOSP2D) order by NOSPM,TGSPM ";
    ...
    <?php 
	$sql = "select KDAKUN,NILMAK from m_spmmak WHERE THANG='$th' and KDSATKER='$kdsatker' and NOSPM='$SPM[NOSPM]' and NOSP2D='$SPM[NOSP2D]' GROUP BY KDAKUN order by KDAKUN";
