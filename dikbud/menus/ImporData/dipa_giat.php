<?php
	...
	$query = "SELECT * FROM $table WHERE THANG='$th' AND KDSATKER='$kdsatker' GROUP BY KDGIAT";
	$total_pages = mysql_num_rows(mysql_query($query));
	#$total_pages = $total_pages[num];
	
	#variabel query
	$targetpage = "index.php?p=$p&kdsatker=$kdsatker&th=$th"; 	#nama file  (nama file ini)
	...
	$oList = mysql_query("SELECT * FROM $table WHERE THANG='$th' AND KDSATKER='$kdsatker' GROUP BY KDGIAT ORDER BY KDGIAT LIMIT $start, $limit");
...
<div align="right">
	<form action="" method="get">
		...
		Cari : 
		<input type="text" name="th" value="<?php echo @$th; ?>" />
		<input type="submit" value="Cari" />
		<a href="http://183.91.67.3/sipristek/index.php?p=33">Reset</a>
	</form>
</div><br />
<table width="650" cellpadding="1" class="adminlist">
	<thead>
	...
	<tbody>
			<tr>
			  <td align="center">&nbsp;</td>
	          <td align="center">&nbsp;</td>
	          <td align="center"><strong><?php echo $kdsatker ?></strong></td>
	          <td align="left"><strong><?php echo nm_satker($kdsatker) ?></strong></td>
	          <td align="right"><strong><?php echo number_format(anggaran_satker($th,$kdsatker),"0",",",".") ?></strong></td>
	          <td align="center">&nbsp;</td>
	  		</tr>
	  <?php
		if ($count == 0) { ?>	
			...
			foreach ($col[0] as $k=>$val) {
				...
				<tr class="<?php echo $class ?>">
					...
					<td width="4%" align="center"><?php echo $col[2][$k] ?></td>
					<td width="42%" align="left"><?php echo nm_giat($col[2][$k]) ?></td>
					<td align="right"><?php echo number_format(anggaran_giat($col[0][$k],$col[2][$k]),"0",",",".") ?></td>
				    <td align="left"><?php echo unit_giat($col[2][$k]) ?></td>
				</tr><?php
			}
		} ?>
	</tbody>
