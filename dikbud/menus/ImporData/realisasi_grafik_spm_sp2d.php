...
<?php
	...
	$sql = "select KDDEPT, KDUNIT from setup_dept ";
			...
			$real_r[$i]  = real_menteri_sdbulan($th,$i);
			if ( $pagu <> 0 )  $real_p[$i]  = $real_r[$i]/$pagu*100;
			else  $real_p[$i]  = 0 ;
			
			}else{
			$real_r[$i]  = real_satker_sdbulan($th,$kdsatker,$i);
			if ( $pagu <> 0 )  $real_p[$i]  = $real_r[$i]/$pagu*100;
			else $real_p[$i]  = 0 ;
			}
			...
			mysql_query("DELETE FROM das_real_ang_sp2d WHERE tahun = '$th' and kdsatker = '$kdsatker'");
			for ( $i = 1 ; $i <= 12 ; $i++ ) 
			{ 
				$pagu_r = 'JML_'.$i ;
				mysql_query("INSERT INTO das_real_ang_sp2d ( tahun , kdsatker , kdbulan , nmbulan , pagu_r , pagu_p , real_r , real_p)
...
$(document).ready(function(){
  var d1 = <?php
	$sql = "select * from das_real_ang_sp2d WHERE tahun = '$th' and kdsatker = '$kdsatker' ORDER BY kdbulan";
...
var d3 = <?php
	$sql = "select * from das_real_ang_sp2d WHERE tahun = '$th' and kdsatker = '$kdsatker' ORDER BY kdbulan";
	$rec3 = mysql_query($sql);
	echo "[";
	$first = true;
	while($row3=mysql_fetch_array($rec3)) {
	if ( $row3['kdbulan'] <= date("m") )  $real_p = $row3['real_p'] ;
	else $real_p = '' ;
		...
		echo "[" . $row3['kdbulan'] . "," . $real_p . "]";
	}
	echo "]";?>
;
  var plot1 = $.jqplot('chart1', [d1, d2, d3], 
    { 
		// title: 'Testing', 
...
<table width="1172" border="0" bgcolor="">
  <tr>
    <td width="571" align="center"><div id="chart1" style="height:315px; width:500px;"></div></td>
    <td width="591">
	
	<table width="438" cellpadding="1" border="1" align="left">
  <thead>
    <tr bgcolor="#6666FF">
      <th width="37" rowspan="3" align="center"><font color="#FFFFFF">BLN</font></th>
      
      <th colspan="6" align="center"><font color="#FFFFFF">Anggaran</font></th>
      </tr>
    <tr bgcolor="#6666FF"> 
      <th colspan="2" align="center"><font color="#FFFFFF">Rencana</font></th>
      <th colspan="2" align="center"><font color="#FFFFFF">Realisasi SP2D</font></th>
      <th colspan="2" align="center"><font color="#FFFFFF">Realisasi SPM</font></th>
      </tr>
    <tr bgcolor="#6666FF">
      <th width="52" align="center">Rp.</th>
      <th width="10" align="center">%</th>
      <th width="34" align="center">Rp.</th>
      <th width="10" align="center">%</th>
      <th width="44" align="center">Rp.</th>
      <th width="10" align="center">%</th>
    </tr>
  </thead>
  <tbody>
	...
	while( $Data = mysql_fetch_array($aData)) { 
		...
		$sql_spm = "select * from das_real_ang_dept WHERE tahun = '$th' and kdsatker = '$kdsatker' and kdbulan = '$Data[kdbulan]' ";
		$aData_spm = mysql_query($sql_spm);
		$Data_spm = mysql_fetch_array($aData_spm);
    ...
    <tr bordercolor="#66CCFF" bgcolor="#E2EAFE">
      ...
      <td align="right"><?php echo number_format($Data['real_r'],"0",",",".") ?></td>
      <td align="center"><?php echo number_format($Data['real_p'],"2",",",".").' %' ?></td>
      <td align="right"><?php echo number_format($Data_spm['real_r'],"0",",",".") ?></td>
      <td align="center"><?php echo number_format($Data_spm['real_p'],"2",",",".").' %' ?></td>
    </tr>
	<?php } ?>
  <tfoot>
  </tfoot>
