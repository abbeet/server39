	...
	{		
		if ($err != true) 
		{
			...
			if ($_FILES['adk']['name'] != "")
			{
				...
				$sql = mysql_query("select * from dt_fileupload_keu WHERE type = 'adk_rkakl' and nama_file = '$filename' and kdsatker = '$kdsatker' ");
				$row = mysql_fetch_array($sql);
				if(empty($row)){
					$sql = "INSERT INTO dt_fileupload_keu (id,kdfile,nama_file,tgl_upload,user_upload,type,kdsatker,keterangan) values ('','ADK','$filename','$tgl_upload','$xusername','adk_rkakl','$kdsatker','File ADK RKAKL')";				
					$query = mysql_query($sql) or die(mysql_error());
				}else{
					$sql = "UPDATE dt_fileupload_keu SET tgl_upload = '$tgl_upload', user_upload = '$xusername' WHERE type = 'adk_rkakl' and kdfile = 'ADK' and kdsatker = '$kdsatker'";				
					$query = mysql_query($sql) or die(mysql_error());
				}
				//--------------------
			...
			$_SESSION['errmsg'] = "Proses Upload ADK RKAKL Satker ".$kdsatker." berhasil"; ?>

			<meta http-equiv="refresh" content="0;URL=index.php?p=608&kdsatker=<?php echo $kdsatker ?>&th=<?php echo $th ?>"><?php
			exit();
...
-->
  ...
  <table width="656" cellspacing="1" class="admintable">
    ...
    <tr> 
      <td class="key">&nbsp;</td>
      <td><strong>Lokasi file pada folder //RKAKL</strong></td>
    </tr>
    <tr> 
      <td class="key">File ADK RKAKL </td>
...
<?php
	$sql = "select * from dt_fileupload_keu WHERE type = 'adk_rkakl' and kdsatker = '$kdsatker' order by kdsatker";
...
<table width="463" cellpadding="1" class="adminlist">
  ...
  <tbody>
    ...
    <?php
			...
			foreach ($col[5] as $k=>$val) {
				if ($k % 2 == 1) $class = "row0";
				else $class = "row1"; 
				?>
    <tr class="<?php echo $class ?>"> 
      <td width="12%" align="center"><?php echo $col[0][$k]?></td>
      <td width="10%" align="left"><a href="file_dipa/<?php echo $col[0][$k]."/".$col[1][$k] ?>" title="Download ADK"><font size="1"> 
        <?php echo $col[1][$k] ?></font></a></td>
