  ...
  <table width="985" cellspacing="1" class="admintable">
    ...
    <tr>
      ...
      <td><select name="kdunitkerja" onchange="get_ikk(this.value,<?php echo $th ?>)">
	  	<?php if ( $q <> '' ) { ?>
        <option value="<?php echo $List['kdunitkerja'] ?>"><?php echo  $List['kdunitkerja'].' '.substr(ket_unit($List['kdunitkerja']),0,70) ?></option>
		<?php } ?>
...
{
        case '3':
					$sql = "SELECT kdunit,left(nmunit,70) as nmunit FROM tb_unitkerja where kdunit = '$xkdunit' ORDER BY kdunit";
		break;
        default:		
					$sql = "SELECT kdunit,left(nmunit,70) as nmunit FROM tb_unitkerja where kdunit <> '480000' and right(kdunit,3) <> '000' ORDER BY kdunit";
		break;
}		
    ...
    <tr>
      ...
      <td><select name="no_sasaran">
        <option value="<?php echo $List['no_sasaran'] ?>"><?php echo  $List['kdunitkerja'].' '.$List['no_sasaran'].' '.substr(nm_sasaran($List['th'],$List['kdunitkerja'],$List['no_sasaran']),0,70) ?></option>
          <option value="" >-- Pilih Sasaran --</option>
          <?php
	  $sql = "SELECT * FROM m_sasaran WHERE ta = '$th' ORDER BY no_sasaran";
					$oSasaran = mysql_query($sql);
					while ($Sasaran = mysql_fetch_array($oSasaran))
					{ ?>
          <option value="<?php echo $Sasaran['no_sasaran']; ?>" <?php if ($Sasaran['no_sasaran'] == $value[6]) echo "selected"; ?>><?php echo $Sasaran['kdunitkerja'].' '.$Sasaran['no_sasaran'].' '.substr($Sasaran['nm_sasaran'],0,70) ?></option>
          <?php
					} ?>
        </select></td>
    </tr>
...
switch ( $xlevel )
{
	  ...
	  break;
					...
					{ ?>
          <option value="<?php echo $IKK['no_ikk']; ?>" style="width:500px;" <?php if ($IKK['no_ikk'] == $value[5]) echo "selected"; ?>><?php echo $IKK['kdunitkerja'].' '.$IKK['no_ikk'].' '.substr($IKK['nm_ikk'],0,70) ?></option>
          <?php
					} ?>
        </select></div></td>
    </tr>
    ...
    <tr> 
      ...
      <td><input type="text" name="no_pk" size="5" value="<?php echo $List['no_pk'] ?>" />&nbsp;&nbsp;<font color="#FF66CC">[isi : 1,2,3,...dst sesuai urutan yang diinginkan]</font></td>
    </tr>
    <tr> 
      <td class="key">Kinerja</td>
      <td><div id="indikator-view"><textarea name="nm_pk" rows="3" cols="50"><?php echo $List['nm_pk'] ?></textarea></div></td>
    </tr>
    <tr>
