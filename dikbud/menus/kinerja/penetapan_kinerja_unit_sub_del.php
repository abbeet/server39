...
//		$sql = sql_delete($table,$field[0],$q);
		...
		if ( $sw == '' ) {
		?>
		<meta http-equiv="refresh" content="0;URL=index.php?p=430&id=<?php echo $_GET['id']; ?>&xkdunit=<?php echo $_REQUEST['xkdunit'] ?>"><?php
		exit();
		}else{ ?>
		<meta http-equiv="refresh" content="0;URL=index.php?p=420&xkdunit=<?php echo $_REQUEST['xkdunit'] ?>"><?php
		exit();
		}
//	}
//	else {
...
<table width="509" cellspacing="1" class="admintable">
	...
	<tr>
	  ...
	  <td><textarea readonly="readonly" cols="60" rows="2"><?php echo nm_unit($PK['kdunitkerja']) ?></textarea></td>
  </tr>
	<tr>
		...
		<td>
			<form name="form" method="post" action="index.php?p=<?php echo $_GET['p'] ?>&id=<?php echo $_GET['id']; ?>&xkdunit=<?php echo $_REQUEST['xkdunit'] ?>&sw=<?php echo $sw ?>">				
				<div class="button2-right">
					<div class="prev">
					<?php if ( $sw == '' ) { ?>
						<a onclick="Back('index.php?p=430&id=<?php echo $_GET['id']; ?>&xkdunit=<?php echo $_REQUEST['xkdunit'] ?>')">Batal</a>
						<?php }else{?>
						<a onclick="Back('index.php?p=420&xkdunit=<?php echo $_REQUEST['xkdunit'] ?>')">Batal</a>
						<?php }?>
					</div>
				</div>
