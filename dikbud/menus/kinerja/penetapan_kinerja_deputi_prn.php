...
//	$arrIndikator   = $pdf->SplitToArray($w1[1],$ln,trim($row['nm_pk']));
	...
	$pdf->SetX($margin);
	$pdf->Cell($w1[0],$ln,'Jumlah Pagu Anggaran '. $th .' :','',1,'L');
	
	if ( substr($kdunit,0,3) == '821' )   $nmprogram = nm_program('0820101');
	else   $nmprogram = nm_program('0820101');
	$pdf->SetX($margin);
	$pdf->Cell(170,$ln,trim($nmprogram).' Rp. '.number_format(pagudipa_deputi($th,$kdunit),"0",",",".").',-','',1,'L');
