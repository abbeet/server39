...
//	$field = get_field($table);
	...
	$oSubRKT = mysql_query("SELECT * FROM $table WHERE  id = '$q' ");
	$SubRKT = mysql_fetch_array($oSubRKT);
...
//		$sql = sql_delete($table,$field[0],$q);
		...
		<meta http-equiv="refresh" content="0;URL=index.php?p=376&xkdunit=<?php echo $_REQUEST['xkdunit'] ?>"><?php
		exit();
		}
//	}
//	else {
...
<table width="509" cellspacing="1" class="admintable">
	...
	<tr>
	  ...
	  <td><textarea readonly="readonly" cols="60" rows="2"><?php echo $SubRKT['nm_rkt_sub'] ?></textarea></td>
  </tr>
	<tr>
	  <td class="key">Target</td>
	  <td><input type="text" size="50" readonly="readonly" value="<?php echo $SubRKT['target'] ?>" /></td>
  </tr>
		...
		<td>
			<form name="form" method="post" action="index.php?p=<?php echo $_GET['p'] ?>&id=<?php echo $_GET['id']; ?>&xkdunit=<?php echo $_REQUEST['xkdunit'] ?>&sw=<?php echo $sw ?>">				
				<div class="button2-right">
					<div class="prev">
					<?php if ( $sw == '' ) { ?>
						<a onclick="Back('index.php?p=390&id=<?php echo $_GET['id']; ?>&xkdunit=<?php echo $_REQUEST['xkdunit'] ?>')">Batal</a>
						<?php }else{?>
						<a onclick="Back('index.php?p=376&xkdunit=<?php echo $_REQUEST['xkdunit'] ?>')">Batal</a>
						<?php }?>
					</div>
				</div>
