...
//			if( $xkdunit <> '' ) {
			...
			if ($_FILES['nmfile']['name'] != "")
			{
				...
				$filedir = "file_lakip/480000/".$th.'_'.$filename ;
				move_uploaded_file($_FILES["nmfile"]["tmp_name"], $filedir);
				//-------- simpan data ------
				$tgl_upload = date ('Y-m-d H:i:s') ;
				$sql = mysql_query("select * from dt_fileupload WHERE th = '$th' and kdunitkerja = '480000' and nourut = '$nourut' and keterangan = 'Lakip' ");
				$row = mysql_fetch_array($sql);
				if(empty($row)){
					$sql = "INSERT INTO dt_fileupload (id,th,kdunitkerja,nourut,dafisi,nama_file,keterangan,tgl_upload,user_upload) values ('','$th','480000','$nourut','$dafisi','$nm_filename','Lakip', '$tgl_upload','$xusername')";				
					$query = mysql_query($sql) or die(mysql_error());
				}else{
					$sql = "UPDATE dt_fileupload SET dafisi = '$dafisi', nama_file = '$nm_filename', tgl_upload = '$tgl_upload', user_upload = '$xusername' WHERE th = '$th' and kdunitkerja = '480000' and nourut = '$nourut' and keterangan = 'Lakip' ";				
					$query = mysql_query($sql) or die(mysql_error());
				}
				//--------------------
...
<?php
	$sql = "select * from dt_fileupload WHERE th = '$th' and kdunitkerja = '480000' and keterangan = 'Lakip' order by nourut";
