...
if ( $xlevel <> 3 and $xlevel <> 4 and $xlevel <> 5 and $xlevel <> 6 and $xlevel <> 7 and $xlevel <> 8 )
	...
	$real_menteri_bulan = real_menteri_spm_bulan($th,$kddept,$kdunit,$kdbulan) ;
	$real_menteri_sdbulan = real_menteri_spm_sdbulan($th,$kddept,$kdunit,$kdbulan) ;
  ...
  ?>
	...
	{
	$real_satker_bulan_akun = real_satker_spm_bulan_akun($th,$col[1][$k],$kdbulan,$Akun['KDAKUN']) ;
	$real_satker_sdbulan_akun = real_satker_spm_sdbulan_akun($th,$col[1][$k],$kdbulan,$Akun['KDAKUN']) ;
	?>
    <tr class="<?php echo $class ?>">
...
<?php 
function real_satker_spm_bulan_akun($th,$kdsatker,$kdbulan,$kdakun) { 
		$sql = "select TGSPM,NILMAK from m_spmmak where THANG = '$th' AND KDSATKER = '$kdsatker' and KDAKUN = '$kdakun'"; 
	...
	function real_satker_spm_sdbulan_akun($th,$kdsatker,$kdbulan,$kdakun) { 
		$sql = "select TGSPM,NILMAK from m_spmmak where THANG = '$th' AND KDSATKER = '$kdsatker' and KDAKUN = '$kdakun'"; 
		$data = mysql_query($sql);
		while( $rdata = mysql_fetch_array($data))
		{
