<?php
	...
	$kddept = setup_kddept_keu($kode) ;
	$kdunit = setup_kdunit_keu($kode) ;

	#Halaman yang akan ditampilkan untuk pertengahan?
	$adjacents = 3;
 
	$query = "SELECT COUNT(*) as num FROM $table WHERE KDDEPT = '$kddept' AND KDUNIT = '$kdunit' AND (NMSATKER LIKE '%$cari%' OR KDSATKER LIKE '%$cari%' OR KDDEPT LIKE '%$cari%' OR KDUNIT LIKE '%$cari%') ";
	...
	$oList = mysql_query("SELECT * FROM $table WHERE KDDEPT = '$kddept' AND KDUNIT = '$kdunit' AND ( NMSATKER LIKE '%$cari%' OR KDSATKER LIKE '%$cari%' OR KDDEPT LIKE '%$cari%' ) ORDER BY KDSATKER LIMIT $start, $limit");
...
<div align="right">
	<form action="" method="get">
		...
		<a href="index.php?p=341">Reset</a>
	</form>
</div><br />
<table cellpadding="1" class="adminlist">
	<thead>
		<tr>
			<th width="4%">No.</th>
			<th width="6%">Kode<br />Satker</th>
			<th width="25%">Nama Satker</th>
			
      <th width="5%">Kode<br />Lembaga/Kementerian</th>
