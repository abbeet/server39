	...
	if (isset($form)) {		
		if ($err != true) {
			...
			if ($q == "") {
				...
				// UPDATE
				$tugas = $value[3] ;
				$sql = "UPDATE $table SET tugas = '$tugas' WHERE id = '$q'";
...
<form action="index.php?p=<?php echo $_GET['p'] ?>" method="post" name="form">
	<table width="647" cellspacing="1" class="admintable">
		
		<tr>
		  <td width="125" class="key">Kode</td>
		  <td width="250" class="row7"><input type="text" name="<?php echo $field[1] ?>" size="10" value="<?php echo $value[1] ?>"  disabled="disabled"></td>
	  </tr>
		<tr>
			<td class="key"> Satker Mandiri </td>
			<td><input type="text" name="<?php echo $field[2] ?>" size="85" value="<?php echo $value[2] ?>" disabled="disabled"/></td>
		</tr>
		<tr>
