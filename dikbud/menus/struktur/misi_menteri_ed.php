<?php
	...
	{		
		...
		{	
			...
			{
				//ADD NEW				
				$sql = "INSERT INTO $table VALUE ('','$kdunit','$kdmisi','$nmmisi')";
				$rs = mysql_query($sql);
				...
				<meta http-equiv="refresh" content="0;URL=index.php?p=490&kdunit=<?php echo $kdunit ?>"><?php
				exit();
			} 
			else {
				// UPDATE
				$sql = "UPDATE $table SET kdmisi = '$kdmisi', nmmisi = '$nmmisi' WHERE id = '$q'";
				$rs = mysql_query($sql);
...
<form action="index.php?p=<?php echo $_GET['p']; ?>&kdunit=<?php echo $kdunit ?>&q=<?php echo $q ?>" method="post" name="form">
	<table width="551" cellspacing="1" class="admintable">
		...
		<tr>
			...
			<td>
			<?php if ( $kdunit <> '' ) {?><input type="text" size="80" value="<?php echo nm_unit($kdunit) ?>" disabled="disabled"/>
			<?php }else{  ?><input type="text" size="80" value="<?php echo nm_unit(@$value['kdunit']) ?>" disabled="disabled"/>
			<?php } ?>
			</td>
		</tr>
		<tr>
		  <td class="key">No. Urut </td>
		  <td><input type="text" name="kdmisi" size="3" value="<?php echo @$value['kdmisi'] ?>" /></td>
	  </tr>
		<tr>
			<td class="key">Misi</td>
			<td><textarea name="nmmisi" cols="60" rows="2"><?php echo @$value['nmmisi'] ?></textarea></td>
		</tr>
		<tr>
			<td>&nbsp;</td>
			<td>			
				<div class="button2-right">
					<div class="prev">
						<a onclick="Back('index.php?p=481')"><?php echo $batal ?></a>					</div>
				</div>
...
<?php
	$sql = "SELECT * FROM $table WHERE kdunit = '$kdunit' ORDER BY kdmisi";
			...
			foreach ($col[0] as $k=>$val) {
				...
				<tr class="<?php echo $class ?>">
					...
					<td width="7%" align="center">
						<a href="index.php?p=490&kdunit=<?php echo $kdunit; ?>&q=<?php echo $col[0][$k]; ?>" title="Edit">
							<img src="css/images/edit_f2.png" border="0" width="16" height="16">						</a>					</td>
					<td width="7%" align="center">
						<a href="index.php?p=491&kdunit=<?php echo $kdunit ?>&q=<?php echo $col[0][$k]; ?>" title="Delete">
