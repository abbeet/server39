	...
	<table class="admintable" cellspacing="1">
		...
		<tr>
			...
			<td>
				<select name="kdunit">
					<option></option><?php
					$oUnit = mysql_query("SELECT kdunit, ket_unit_kerja FROM tb_unitkerja ORDER BY ket_unit_kerja");
					
					while ($Unit = mysql_fetch_array($oUnit))
					{ ?>
							<option value="<?php echo $Unit['kdunit']; ?>" <?php if ($Unit['kdunit'] == $value[7]) echo "selected"; ?>><?php echo $Unit['ket_unit_kerja']; ?></option><?php
					} ?>
				</select>
			</td>
		</tr><?php
